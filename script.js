...
$(document).ready(function () {
  ...
  // update property on display for every unique cell
...
// updated cells ki property change kardega
function updateCell(property, value, defaultPossible) {
  $(".input-cell.selected").each(function () {
    $(this).css(property, value);
    let [rowId, colId] = getRowCol(this);

    // check if cell already exists in "cellData"
    // exist: we need to update it
    // not exist: we need to add in "cellData"
    if (cellData[selectedSheet][rowId]) {
      if (cellData[selectedSheet][rowId][colId]) {
        // update
        cellData[selectedSheet][rowId][colId][property] = value;
      } else {
        // row exists but col does not: therefore we add cell
    ...
    // Now if after updation, the cell properties are back to default then we need to delete it from "cellData"
...
// This will fill "selectedCells" with selected cell's (rowID,colID)
